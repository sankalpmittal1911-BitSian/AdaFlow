    ...
    // #include "/home/tofino/Open-Tofino/p4-examples/p4_16_programs/common/util.p4"
        ...
        *************************************************************************/
        ...
        parser SwitchIngressParser(packet_in packet,
         ...
         //For real traffic analysis, do not parse timestamp; instead use meta.now_timestamp = (bit<32>) ig_prsr_md.global_tstamp>>20; // ms
            state timestamp {
        ...
        *************************************************************************/
        ...
        inout ingress_intrinsic_metadata_for_tm_t ig_tm_md)
        {
            ...
            @symmetric("meta.srcPort", "meta.dstPort")
...
Register<bit<32>, bit<16>>(REGISTER_LENGTH, 0) ipd_min;
            ...
            table shift_bit {
                ...
                Register<bit<16>, bit<16>>(REGISTER_LENGTH, 0) feature1;
     
                //Update packet count
                RegisterAction<bit<16>, bit<16>, bit<16>>(feature1) feature1_action = {
                    void apply(inout bit<16> n_pkts, out bit<16> curr_pkts) {
                            bit<16> temp = n_pkts;
                            if(meta.reset){
                                n_pkts = 1;
                ...
                action feat1()
                {
                   meta.f1 = feature1_action.execute(meta.flowlet_register_index);
                }
Register<bit<16>, bit<16>>(REGISTER_LENGTH, 0) feature2;
     
                //Update packet count
                RegisterAction<bit<16>, bit<16>, bit<16>>(feature2) feature2_action = {
                    void apply(inout bit<16> n_pkts, out bit<16> curr_pkts) {
                            bit<16> temp = n_pkts;
                            if(meta.reset){
                                n_pkts = 1;
                ...
                action feat2()
                {
                    meta.f2 = feature2_action.execute(meta.flowlet_register_index);
                }

                Register<bit<16>, bit<16>>(REGISTER_LENGTH, 0) feature3;
     
                //Update packet count
                RegisterAction<bit<16>, bit<16>, bit<16>>(feature3) feature3_action = {
                    void apply(inout bit<16> n_pkts, out bit<16> curr_pkts) {
                            bit<16> temp = n_pkts;
                            if(meta.reset){
                                n_pkts = 1;
                ...
                action feat3()
                {
                    meta.f3 = feature3_action.execute(meta.flowlet_register_index);
                }

                Register<bit<16>, bit<16>>(REGISTER_LENGTH, 0) feature4;
     
                //Update packet count
                RegisterAction<bit<16>, bit<16>, bit<16>>(feature4) feature4_action = {
                    void apply(inout bit<16> n_pkts, out bit<16> curr_pkts) {
                            bit<16> temp = n_pkts;
                            if(meta.reset){
                                n_pkts = 1;
                ...
                action feat4()
                {
                    meta.f4 = feature4_action.execute(meta.flowlet_register_index);
                }
...
meta.srcPort = hdr.tcp.srcPort;
            ...
            meta.new_head.ipd_max = meta.ipd_max;
            meta.new_head.size_avg = meta.size_avg;
            meta.new_head.size_min = meta.size_min;
            meta.new_head.size_max = meta.size_max;
            meta.new_head.f1 = meta.f1;
            meta.new_head.f2 = meta.f2;
            meta.new_head.f3 = meta.f3;
            meta.new_head.f4 = meta.f4;
